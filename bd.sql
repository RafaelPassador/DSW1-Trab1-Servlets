...
use SistemaVeiculos;
create table Cliente(id bigint not null auto_increment, email varchar(512) not null unique, pass varchar(21844) not null, cpf char(11) not null, nome varchar(256) not null, telefone varchar(14) not null, sexo varchar(1) not null, nascimento date not null, primary key(id));
create table Loja(id bigint not null auto_increment, email varchar(512) not null unique, pass varchar(21844) not null, cnpj char(14) not null, nome varchar(256) not null, descricao varchar(1024) not null, primary key(id));
...
create table Imagens(id bigint not null auto_increment, carro_id char(7) not null, endereco varchar(21844) not null, primary key(id), foreign key(carro_id) references Carros(id));
create table Proposta(id bigint not null auto_increment, loja_id bigint not null, cliente_id bigint not null, valor float not null, condicoes varchar(21844) not null, estado varchar(11) not null, primary key(id), foreign key(cliente_id) references Cliente(id), foreign key(loja_id) references Loja(id));
